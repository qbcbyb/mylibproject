 ...
 * <p/>
 * <p>Item views may span multiple columns as specified by their {@link com.origamilabs.library.views.StaggeredGridView.LayoutParams}.
...
public class StaggeredGridView extends ViewGroup {
    ...
    private void recycleOffscreenViews() {
        ...
        if (childCount > 0) {
            int[] oldItemTops = Arrays.copyOf(mItemTops, mItemTops.length);
            int[] oldItemBottoms = Arrays.copyOf(mItemBottoms, mItemTops.length);
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                for (int col = lp.column; col < colEnd; col++) {
                    ...
                    if (colTop < mItemTops[col]) {
                        ...
                        Log.d("mItemTops[" + col + "] = colTop", col + ":" + colTop);
                    }
...
//                    mItemTops[col] = 0;
//                    mItemBottoms[col] = 0;
//                    Log.d("mItemTops[" + col + "] = 0", col + ":" + oldItemTops[col] + ":" + oldItemBottoms[col]);
                    mItemTops[col] = oldItemBottoms[col];
                    mItemBottoms[col] = oldItemBottoms[col];
                    Log.d("mItemTops[" + col + "] = 0", col + ":" + oldItemTops[col]);
                }
            }
        }
    }
...
//        drawSelector(canvas);
    ...
    private void populate(boolean clearData) {
        ...
        for (int i = 0; i < colCount; i++) {
            ...
            Log.d("mItemTops[" + i + "] = (offset == 0) ? mItemTops[i] : offset;", i + ":" + mItemTops[i]);
    ...
    final void offsetChildren(int offset) {
        ...
        for (int i = 0; i < colCount; i++) {
            mItemTops[i] += offset;
            Log.d("mItemTops[" + i + "] += offset;", i + ":" + mItemTops[i]);
...
//            }
            ...
            for (int i = nextCol; i < nextCol + span; i++) {
                ...
                Log.d("mItemTops[" + i + "] = childTop - rec.getMarginAbove(i - nextCol) - itemMargin;", i + ":" + mItemTops[i]);
            }
